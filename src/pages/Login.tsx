...
const Login = () => {
  ...
  const { user, role, isLoading: isAuthLoading } = useAuth(); // Hook do contexto global

  // Efeito para redirecionar assim que o usuário e a role estiverem carregados no Contexto
  useEffect(() => {
    if (!isAuthLoading && user && role && selectedModule) {
      handleRedirect(role);
    }
  }, [user, role, isAuthLoading, selectedModule, navigate]);
  ...
  const handleRedirect = (userRole?: string) => {
    ...
    } else if (targetRole === "safety_tech") {
      navigate("/epi/dashboard", { replace: true });
    } else if (targetRole === "almoxarifado") {
      navigate("/stock/dashboard", { replace: true });
    } else if (targetRole === "pcm") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        // Não chamamos handleRedirect aqui manualmente.
        // O useEffect vai detectar a mudança no useAuth e redirecionar automaticamente.
      }
    } catch (error: any) {
      toast({
